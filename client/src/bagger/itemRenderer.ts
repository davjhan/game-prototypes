...
import { Placeable } from '$bagger/items'

const cornerRadius = 4
export function render(item: Placeable, size: Size, className:string) {
	const points = walkAlongEdge(item, cornerRadius/size.cell)
	// console.log(`%c Points: ${ JSON.stringify(points) }`, 'color:yellow')

	const path = points.join(' ')
	return `
	<svg width='${ item.layout[0].length * size.cell }' height= '${ item.layout.length * size.cell }'
